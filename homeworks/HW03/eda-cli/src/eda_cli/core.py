...
def compute_quality_flags(
    ...
    # Проверка на категориальные признаки с высокой кардинальностью
    has_high_cardinality_categoricals = False
    HIGH_CARDINALITY_THRESHOLD = max(50, n_rows * 0.1)  # Порог зависит от размера датасета
    
    for col in summary.columns:
        if not col.is_numeric and col.unique > HIGH_CARDINALITY_THRESHOLD and col.unique > 10:
            has_high_cardinality_categoricals = True
            break
    
    # Проверка на проблемные колонки с пропусками
    ...
    if flags["problematic_missing_count"] > 0:
        penalty = min(0.3, flags["problematic_missing_count"] * 0.05)
        score -= penalty
