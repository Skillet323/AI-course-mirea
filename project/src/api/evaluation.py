...
from ..services.evaluation import build_proxy_metrics, create_gold_standard, evaluate_meeting
...
def evaluate_meeting_endpoint(meeting_id: int, session: Session = Depends(get_session)):
    ...
    try:
        ...
        proxy = build_proxy_metrics(meeting, session)
        return {
            "evaluation_run_id": None,
            "overall_quality_score": proxy["overall_quality_score"],
            "metrics": proxy,
            "mode": "proxy",
        }
