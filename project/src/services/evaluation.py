...
DEFAULT_GOLD_DIRS = [
    ...
    PROJECT_ROOT / "gold_annotations",
    PROJECT_ROOT / "data" / "gold_annotations",
]


def _load_gold_json_from_disk(candidate_refs: List[str], gold_dirs: Optional[List[Path]] = None) -> Optional[dict]:
    dirs = gold_dirs or DEFAULT_GOLD_DIRS
    for base in dirs:
        if not base.exists():
            continue
...
def _ensure_gold_for_meeting(session: Session, meeting: Meeting) -> Optional[GoldStandard]:
    gold = _find_gold_for_meeting(session, meeting)
    if gold:
        return gold

    disk_payload = _load_gold_json_from_disk(_meeting_ref_candidates(meeting))
    if not disk_payload:
        return None
